...
func TestCheckCalls(t *testing.T) {
	reader1 := NewReaderMock()
	reader1.RegisterRead(func(p []byte) (n int, err error) {
		return 0, nil
	})
	reader2 := NewReaderMock()
	reader2.RegisterRead(func(p []byte) (n int, err error) {
		return 0, nil
	}).RegisterRead(func(p []byte) (n int, err error) {
		return 0, nil
	})
	m := CheckCalls([]*Mock{reader1.Mock, reader2.Mock})
	if len(m) != 2 {
		t.Error("unexpected CheckCalls result")
	}
	arr, pst := m[0]
	...
	err := checkMethodCallsInfo(arr[0], 1, 0)
	if err != nil {
		t.Error(err)
	}
	arr, pst = m[1]
	...
	checkMethodCallsInfo(arr[0], 2, 0)
	if err != nil {
		t.Error(err)
	}
}
